...
# Extension repo to monitor for version changes
EXT_GITHUB_REPO="DigitalCyberSoft/php-boringssl"
...
check_ext_version() {
    ...
    new_ver=$(curl -sf "${auth[@]}" --connect-timeout 15 --max-time 60 --retry 3 --retry-all-errors "https://api.github.com/repos/${EXT_GITHUB_REPO}/releases/latest" 2>/dev/null \
...
${changes}
This commit triggers automatic COPR rebuild.
...
if [ $VERSIONS_CHANGED -eq 1 ]; then
    ...
    if [ "$MODE" = "update" ]; then
        if [ $DRY_RUN -eq 1 ]; then
            ...
            # Only bump release for PHP changes; extension version changes already reset it
